...
export async function getPeserta() {
  if (!BASE_URL) return [];
  try {
    const res = await fetch(`${BASE_URL}?sheet=peserta`);
    if (!res.ok) throw new Error(`API error ${res.status}`);
    const data = await res.json();
    return Array.isArray(data.peserta) ? data.peserta : [];
  } catch (err) {
...
export async function getHasil() {
  if (!BASE_URL) return [];
  try {
    const res = await fetch(`${BASE_URL}?sheet=hasil`);
    if (!res.ok) throw new Error(`API error ${res.status}`);
    const data = await res.json();
    return Array.isArray(data.hasil) ? data.hasil : [];
  } catch (err) {

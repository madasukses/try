...
export async function catatPeserta({ nama, email, waktuMulai }) {
  if (!BASE_URL) {
    console.info('[demo] peserta dicatat lokal:', nama, email);
  ...
  try {
    const res = await fetch(`${BASE_URL}/peserta`, {
      ...
      body: JSON.stringify({ peserta: { nama, email, waktuMulai } }),
    });
    if (!res.ok) throw new Error(`Sheety error ${res.status}`);
    return res.json();
  } catch (err) {
...
export async function kirimHasil({ nama, email, skor, benar, salah, kosong, waktuSelesai, detailJawaban }) {
  if (!BASE_URL) {
    console.info('[demo] hasil dicatat lokal:', { nama, skor });
    return { id: `demo-${Date.now()}` };
  }
  try {
